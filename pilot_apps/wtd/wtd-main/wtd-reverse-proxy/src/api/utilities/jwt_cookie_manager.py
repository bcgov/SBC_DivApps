...
"""JwtCookieManager handles validating JWTs using a minimal configuration for an OIDC server.
...
class JwtCookieManager:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, app=None):
        """Initialize the JwtCookieManager instance."""
        ...
        self.algorithms = JwtCookieManager.ALGORITHMS
        self.jwks_uri = None
        self.issuer = None
        self.audience = None
    ...
    def init_app(self, app):
        ...
        if self.jwt_oidc_test_mode:
            ...
            self.algorithms = [app.config.get(
                'JWT_OIDC_ALGORITHMS', JwtCookieManager.ALGORITHMS)]
        ...
        auth_err_handler = app.config.get(
            'JWT_OIDC_AUTH_ERROR_HANDLER', JwtCookieManager.handle_auth_error)
    ...
    def get_token_auth_header():
        header = request.headers.get('Cookie', None)
        if not header:
        ...
        if header:
            ...
            if cookies:
                for cookie in cookies:
                    ...
                    if strippedCookie.startswith(JwtCookieManager.COOKIE_PREFIX):
                        return strippedCookie[len(JwtCookieManager.COOKIE_PREFIX):]
        raise AuthError({'code': 'authorization_header_missing',
                        'description': 'Cannot read authorization cookie'}, 401)
