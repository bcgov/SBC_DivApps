...
public class SimpleDBDataPipeline extends AbstractDataPipeline {
    ...
    public Map<String, Object> prepare(Map<String, Object> variables) {
        ...
        for (Map.Entry<String, Object> entry : variables.entrySet()) {
            if (entry.getValue() != null) {
                if (StringUtils.endsWith(entry.getKey(), "_date")
                        || StringUtils.endsWith(entry.getKey(), "_date_time")) {
                    if (entry.getKey().equals("action_claim_date_time")) {
                        try {
                            Timestamp timestamp = new Timestamp(Long.parseLong(String.valueOf(entry.getValue())));
                    ...
                    } else if (entry.getValue() != null && !"null".equalsIgnoreCase(String.valueOf(entry.getValue()))
    ...
    /**
     * Implementation method for direct database connectivity with downstream
     * system.
     * This method handles lob & non-lob objects separately to keep the thread span
     * short.
     *
     * @param data
     * @return
            ...
            for (Map.Entry<String, Object> entry : data.entrySet()) {
                if (StringUtils.endsWith(entry.getKey(), "_file")) {
                    ...
                    if (!lobMap.containsKey(entry.getKey())) {
                        ...
                        lobData.put("name", getDateWithoutSpecialCharacters(
                                String.valueOf(data.get(fileNamePrefix.concat("_name")))));
                        ...
                        // nonLobMap.put(StringUtils.concat("_id"),
                        // data.get(fileNamePrefix.concat("_stream_id")));
            ...
            for (Map.Entry<String, Object> entry : data.entrySet()) {
                if (StringUtils.endsWith(entry.getKey(), "_uploadname")) {
                    if (entry.getValue() != null && StringUtils.isNotBlank(String.valueOf(entry.getValue()))
                        ...
                        nonLobMap.put(StringUtils.substringBefore(entry.getKey(), "_file_uploadname") + "_file_id",
    ...
    private static String getDateWithoutSpecialCharacters(String filename) {
        String timestampVal = StringUtils.replace(StringUtils.replace(
                StringUtils.replace(
                        StringUtils.replace(StringUtils.substringBefore(new DateTime().toString(), "."), "-", ""), ":",
                        ""),
                ".", ""), " ", "");
    ...
    private void handleFileObject(Map<String, Map<String, Object>> lobMap) throws SQLException {
        for (Map.Entry<String, Map<String, Object>> entry : lobMap.entrySet()) {
            String query = getQuery(String.valueOf(entry.getValue().get("files_entity_key")), entry.getValue(),
                    "stream_id", String.valueOf(entry.getValue().get("stream_id")));
     ...
     * @throws SQLException
     */
    private String getQuery(String formKey, Map<String, Object> dataMap, String pkname, String pkvalue)
            throws SQLException {
        ...
        return IQueryFactory.prepareQuery(formKey, filteredCols,
                StringUtils.isEmpty(getIdentityKey(cols)) ? Boolean.FALSE : Boolean.TRUE, pkname);
    }
    ...
    private Map<String, Object> getColumns(String formKey, String pkName, String pkValue) throws SQLException {
        ...
        Map<String, Object> resp = analyticsJdbcTemplate.query(getValidationQuery(formKey, pkName), namedParameters,
