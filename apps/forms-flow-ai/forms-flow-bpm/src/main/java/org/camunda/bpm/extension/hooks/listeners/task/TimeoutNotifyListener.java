...
public class TimeoutNotifyListener extends BaseListener implements TaskListener, IMessageEvent {
    ...
    public void notify(DelegateTask delegateTask) {
        DateTime currentDate = this.currentDate != null && this.currentDate.getValue(delegateTask) != null
                ? new DateTime(String.valueOf(this.currentDate.getValue(delegateTask)))
                : new DateTime();
        ...
        String escalationGroup = this.escalationGroup != null && this.escalationGroup.getValue(delegateTask) != null
                ? String.valueOf(this.escalationGroup.getValue(delegateTask))
                : null;
    ...
    private void validateAssigneeAndNotify(DelegateTask delegateTask, String category, String escalationGroup) {
        ...
        if (StringUtils.isNotEmpty(delegateTask.getAssignee())) {
            // LOGGER.log(Level.INFO, "GetAssignee" + delegateTask.getAssignee());
            log.info("GetAssignee" + delegateTask.getAssignee());
            ...
            if (StringUtils.isNotEmpty(user.getEmail())) {
                ...
                sendEmailNotification(delegateTask.getExecution(), category, userEmails, escalationGrpEmails,
                        delegateTask.getId());
            }
        } else {
    ...
    private void sendEmailNotification(DelegateExecution execution, String category, List<String> toEmails,
            List<String> ccEmails, String taskId) {
